...
import com.angesh.quizappassignment7.databinding.FragmentResultBinding
import com.angesh.quizappassignment7.viewModel.AnswerViewModel
import com.angesh.quizappassignment7.ui.SplashActivity
...
class ResultFragment : Fragment(R.layout.fragment_result), OnClickListener {
    ...
    private fun initData(totalScore: Int) {
        ...
        binding.wrongAnswerTv.text = getString(R.string.incorrect_answers) + " " + (sizeOfData - totalScore)
    ...
    override fun onClick(v: View?) {
        when(v!!.id) {
            ...
            R.id.result_analysis_button -> {
                val intent = Intent(activity, DetailReportActivity::class.java)
                startActivity(intent)
            }
        }
    }
    ...
    private fun resetDB() {
        viewModel.resetDatabase(QuizList.getQuizQuestions())
    }
}